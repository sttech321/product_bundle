...
$selected_products = get_post_meta($post->ID, 'select_product', true);
  ...
  if ($query->have_posts()) {
    while ($query->have_posts()) {
      ...
      echo '<span><img src="' . get_the_post_thumbnail_url() . '" alt="' . get_the_title() . '" style="width:80px;"></span>';
      
      $checked = in_array(get_the_id(), (array) $selected_products) ? 'checked="checked"' : '';
        ...
        if (searchQuery !== '') {
          $.ajax({
            ...
            success: function(response) {
              $('#bundle_product_options').html(response);
            },
...
function wp_save_custom_tab_data($post_id){
  ...
  // Checkbox values are sent as an array, so ensure we get an array
...
function display_selected_products_in_cart($product_name, $cart_item) {
  ...
  if (!empty($selected_products)) {
      ...
      $product_name .= '<br><em class="cart" style="background-color:white; border:1px solid black;">Selected Products: ' . implode(', ', $selected_product_names) . '</em>';
  }
